...
public class Brush : MonoBehaviour {
    ...
    private Vector3 _handExpressionPosition;
    HandJointPose handExpressionPose;
    ...
    private void Update() {
        ...
        XRNode nodeExpression = _hand == Hand.LeftHand ? XRNode.RightHand: XRNode.LeftHand;
        if(XRSubsystemHelpers.HandsAggregator.TryGetJoint(TrackedHandJoint.IndexTip, nodeExpression, out handExpressionPose))
        {
            _handExpressionPosition = handExpressionPose.Position;
        }
        else
        {
            _handExpressionPosition = Vector3.one;
        }

        //Debug.Log(Mathf.Abs(Camera.main.transform.position.y - _handExpressionPosition.y));
        /*
         * 
         * 
                Debug.Log("node: " + node.ToHandedness());
                Debug.Log("nodeExpression: " + nodeExpression.ToHandedness());
        */
        ...
        //hazza
        XRSubsystemHelpers.HandsAggregator.TryGetJoint(TrackedHandJoint.IndexTip, node, out poseIndexTip);

        // Get the position & rotation of the hand
        //bool handIsTracking = UpdatePose(node, ref _handPosition, ref _handRotation);

        bool handIsTracking = XRSubsystemHelpers.HandsAggregator.TryGetJoint(TrackedHandJoint.IndexTip, node, out poseIndexTip);

        _handPosition = poseIndexTip.Position;
        _handRotation = poseIndexTip.Rotation;
        ...
        if (triggerPressed && _activeBrushStroke == null) {
            ...
            float width = map(_handExpressionPosition.y, 0.5f, 2.0f, 0.01f, 0.2f);
            _activeBrushStroke._mesh._brushStrokeWidth = width;
            Debug.Log(width);

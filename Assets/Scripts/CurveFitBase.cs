...
{
    protected const FLOAT EPSILON = 1.2e-12f;  // below this, we can't trust floating point values
    ...
    protected VECTOR GetCenterTangent(int first, int last, int split)
    {
        ...
        UnityEngine.Debug.Assert(first < split && split < last);
    ...
    protected void InitializeArcLengths()
    {
        ...
        List<FLOAT> arclen = _arclen;
        int count = pts.Count;
        UnityEngine.Debug.Assert(arclen.Count == 0);
        arclen.Add(0);
        FLOAT clen = 0;
        VECTOR pp = pts[0];
    ...
    protected void Reparameterize(int first, int last, CubicBezier curve)
    {
        ...
        for (int i = 1; i < nPts; i++)
        {
            ...
            FLOAT num = ((VectorHelper.GetX(p0) - VectorHelper.GetX(p)) * VectorHelper.GetX(p1)) + ((VectorHelper.GetY(p0) - VectorHelper.GetY(p)) * VectorHelper.GetY(p1)) + ((VectorHelper.GetZ(p0) - VectorHelper.GetZ(p)) * VectorHelper.GetZ(p1));
            FLOAT den = (VectorHelper.GetX(p1) * VectorHelper.GetX(p1)) + (VectorHelper.GetY(p1) * VectorHelper.GetY(p1)) + (VectorHelper.GetZ(p1) * VectorHelper.GetZ(p1)) 
                + ((VectorHelper.GetX(p0) - VectorHelper.GetX(p)) * VectorHelper.GetX(p2)) + ((VectorHelper.GetY(p0) - VectorHelper.GetY(p)) * VectorHelper.GetY(p2)) + ((VectorHelper.GetZ(p0) - VectorHelper.GetZ(p)) * VectorHelper.GetZ(p2));
